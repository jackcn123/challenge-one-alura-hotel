...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		lblFormaPago.setFont(new Font("Dialog", Font.BOLD, 16));
		...
		JLabel lblValor = new JLabel("VALOR DE LA RESERVA (S/.)");
		...
		((JTextField) txtFechaEntrada.getDateEditor()).setEditable(false); 
		...
		((JTextField) txtFechaSalida.getDateEditor()).setEditable(false);
		...
		txtValor.setBounds(78, 328, 72, 33);
		...
		btnsiguiente.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (txtFechaEntrada.getDate() != null && txtFechaSalida.getDate() != null) {
					String fpago = (String) txtFormaPago.getSelectedItem();
					Date fechain = Date
							.valueOf(((JTextField) txtFechaEntrada.getDateEditor().getUiComponent()).getText());
					Date fechaout = Date
							.valueOf(((JTextField) txtFechaSalida.getDateEditor().getUiComponent()).getText());
					Double valor_reserva = Double.valueOf(txtValor.getText());
					
					Reservas reservas = new Reservas(fechain, fechaout, valor_reserva, fpago);
					reservasController.guardar(reservas);
					RegistroHuesped registro = new RegistroHuesped(String.valueOf(reservas.getId()));
					registro.setVisible(true);
					dispose();
				} else {
					JOptionPane.showMessageDialog(null, "No es posible Continuar, Datos incompletos");
				}
			}
		});
		btnsiguiente.setName("");
	...
	protected void calcularValor(JDateChooser txtFechaEntrada2, JDateChooser txtFechaSalida2) {
		double tasa = 80.00;
		double valor;

		if (txtFechaEntrada2.getDate() != null && txtFechaSalida2.getDate() != null) {
			Date fechain = Date.valueOf(((JTextField) txtFechaEntrada.getDateEditor().getUiComponent()).getText());
			Date fechaout = Date.valueOf(((JTextField) txtFechaSalida.getDateEditor().getUiComponent()).getText());

			long tiempoTranscurrido = fechaout.getTime() - fechain.getTime();
			TimeUnit unidad = TimeUnit.DAYS;
			long dias = unidad.convert(tiempoTranscurrido, TimeUnit.MILLISECONDS);
			valor = tasa * dias;
			if (dias <= 0) {
				JOptionPane.showMessageDialog(this, "Verificar las fechas seleccionadas");
